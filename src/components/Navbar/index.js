...
const YibanNavbar = () => {
  ...
  useEffect(() => {
    if (account) {
      axios.get(`http://localhost:5000/api/user/${JSON.parse(localStorage.getItem("currentAccount")).address}`)
        .then(res => setVerified(res.data.isVerified === 'true' ? true : false))
    }
  }, [account])
        ...
        <div className="block-flex ">
          ...
          <div className=" mt-1 text-center mr-5">
            ...
            <Nav.Link href="/settings" className="text-center">
              <h5 className={`account__name${verified ? '--verified' : ''}`}>{account ? account.meta.name : ""}</h5>
            </Nav.Link>
          </div>
        </div>
      </Nav>
